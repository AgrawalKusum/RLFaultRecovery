...
def test(model, env, num_procs, num_episodes=None):
  ...
  print(f"DEBUG: VecEnv Observation Space: {env.observation_space}")
  try:
    o = env.reset()
    print(f"Success! Shape: {o.shape}")
  except Exception as e:
      print(f"CRASH DETECTED: {e}")
      # Manually inspect the internal venv result
      raw_obs = env.venv.reset() 
      print(f"Raw obs from Venv: {raw_obs}")
...
def make_env(args, enable_env_rand):
    def _init():
        ...
        old_reset = raw_env.reset

        def patched_reset(**kwargs):
            val = old_reset(**kwargs)
            if isinstance(val, tuple):
                return val
            return val, {}
        raw_env.reset = patched_reset

        old_step = raw_env.step
        def patched_step(action):
            val = old_step(action)
            if isinstance(val, tuple) and len(val) == 4:
                obs, rew, done, info = val
                return obs, rew, done, False, info
            return val
        raw_env.step = patched_step
        
        # Wrap for Gymnasium compatibility
        #env = EnvCompatibility(raw_env, render_mode=None)
        #env.new_step_api = False
        #env.new_reset_api = False
...
def main():
  ...
  env = DummyVecEnv([make_env(args, enable_env_rand) for _ in range(num_procs)])
