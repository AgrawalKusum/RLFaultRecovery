...
try:
    ...
    numpy_include = []
    print("Warning: numpy not found. Compilation may fail or be slow.")
...
# Source files for the MPC solver
sources = [
    "mpc_controller/mpc_osqp.cc",
    "third_party/osqp/src/auxil.c",
    "third_party/osqp/src/cs.c",
...
cxx_flags = ['-D__SUPPRESSANYOUTPUT__', '-DPYBULLET_USE_NUMPY']
libraries = []

if sys.platform.startswith('linux'):
    include_dirs += ['third_party/osqp/include/linux']
    libraries = ['dl', 'pthread']
    cxx_flags += ['-fpermissive', '-D_LINUX', '-std=c++17', '-Wno-sign-compare', '-fvisibility=hidden']
elif sys.platform == 'darwin':
    include_dirs += ['third_party/osqp/include/macosx']
    cxx_flags += ['-std=c++17', '-mmacosx-version-min=10.7', '-stdlib=libc++']
elif sys.platform == 'win32':
    include_dirs += ['third_party/osqp/include/windows']
    libraries = ['User32', 'kernel32']
    cxx_flags += ['/std:c++17', '-DWIN32']
